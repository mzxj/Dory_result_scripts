...
for (i in 1:length(lists)) {
  assign(objs[i],read.table(paste0("SourceData/PgreatPless/",lists[i]), sep="\t", header = TRUE)) ##assign 函数 Assign a Value to a Name
}
...
correcords <- c()
for(sub in suball){
  dirsub <- paste0('cor_sub', sub)
  if (!dir.exists(dirsub)) {
    dir.create(dirsub)
  }
  correcord <- c()
  for(k in 1:20){
    data0 <- read.table(paste0("SourceData/sub", sub, "/Rep4_WpCauchyPGreatLess_rep4ct", cti,"sub", sub, "random", k, "VSrep4ct", ctj,"sub", sub, "random", k, ".tsv"), header = TRUE)
    ...
    plt <- ggplot(data, aes(x=sub358, y=whole, color=I(PointDense)))+
      ...
      stat_cor(aes(group=1),method = "pearson", label.x.npc = "left", label.y.npc = "top", size=3)

    pdf(paste0(dirsub, '/Pearson_whole_sub', sub,'case', k, '.pdf'), width = 2.2, height = 2.2)
    print(plt)
    dev.off()
    correcord <- cbind(correcord, cor(data$sub358, data$whole, method = "pearson", use = "complete.obs"))
  }
  correcords <- rbind(correcords, correcord)
}
